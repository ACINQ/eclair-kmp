                ...
                when (msg) {
                    ...
                    is PeerStorageRetrieval -> {
                        if (nodeParams.usePeerStorage) {
                            ...
                            suspend fun recoverChannel(recovered: PersistedChannelState): ChannelState {
                            ...
                            when {
                                ...
                                backup is DeserializationResult.Success -> {
                                    ...
                                    val (state1, actions1) = state.process(ChannelCommand.PeerBackupReceived)
                                    processActions(backup.state.channelId, peerConnection, actions1)
                                    _channels = _channels + (backup.state.channelId to state1)
                                }
                            }
                        } else {
